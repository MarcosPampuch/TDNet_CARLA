...
#        return file_path
#    else:
#        raise ValueError('Downloaded file has different hash. Please try again.') #https://download.pytorch.org/models/resnet101-5d3b4d8f.pth
    ####################################################################################################
    file_name = 'resnet101s-03a0f310'
    root = os.path.expanduser(root)
    file_path = os.path.join(root, file_name+'.pth')
#    sha1_hash = _model_sha1[name]
#    if os.path.exists(file_path):
...
#    if repo_url[-1] != '/':
#        repo_url = repo_url + '/'
    download('https://github.com/yassouali/pytorch-segmentation/releases/download/v0.1/resnet101s-03a0f310.zip',
             path=zip_file_path,
             overwrite=True)
    with zipfile.ZipFile(zip_file_path) as zf:
        zf.extractall(root)
...
#
#    download('https://download.pytorch.org/models/resnet101-63fe2227.pth',
#             path=zip_file_path,
#             overwrite=True)
#
#    return file_path
